...
void fetchAndExecuteCommandsImmediate()
{
  ...
  if (httpResponseCode == 200)
  {
    ...
    // The backend expires stale records and returns at most five fresh
    // commands, keeping this parser and the ESP32 workload bounded.
    ...
    else
    {
      ...
      if (responseDoc["has_more"] | false)
      {
        Serial.println("[Commands] More fresh commands available - queueing next batch");
        enqueueFetchCommands();
      }
    }
  }
