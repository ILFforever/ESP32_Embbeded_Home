...
const enrollFace = async (req, res) => {
  try {
    ...
    // Prepare command parameters (face_id is generated by the doorbell device)
    const params = {
      expireAt: Math.floor(expiresAt.getTime() / 1000) // Unix timestamp in seconds
    };
    ...
    console.log(`[FaceManagement] ${device_id} - Enroll face command queued (ID: ${commandRef.id}, expires at: ${expiresAt.toISOString()})`, params);
...
const handleNfcAccessScan = async (req, res) => {
  try {
    ...
    // 4. If authorized, trigger an action (e.g., unlock the door 'dl_001')
    if (authorized) {
      ...
      if (unlockResult.success) {
        console.log(`[NFC Access] Unlock command queued for ${targetDeviceId} for user ${userName}`);
        return res.json({
          status: 'ok',
        ...
        console.error(`[NFC Access] Failed to queue unlock command for ${targetDeviceId}:`, unlockResult.error);
        return res.status(500).json({
          status: 'error',
